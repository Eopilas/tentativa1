...
void LogInit()
{
    ...
    fprintf(g_logFile,
        ...
        "  CIVICO_DRIVE_1: speed_ap, physSpeed, playerSpeed, dist, distToDest,\n"
        "             mission(nome), style(nome), tempAction(nome), heading, dest(xyz),\n"
        "             aggr, modo, onRoad, offroad, playerOffroad, playerRoadDist,\n"
        "             curveBrake(0/1), deltaH(rad) — v5.7: curve brake restaurado\n"
        "  CIVICO_DRIVE_2: offroad, offroadSust, stuck, headon, linkId,\n"
        "             playerOffroadDirect, wasOffroadDirect, targetCar, tasks\n"
        "  physSpeed: velocidade fisica real (m_vecMoveSpeed x 180 ≈ km/h)\n"
        "  speed_ap:  velocidade de cruzeiro do AutoPilot (unidades SA)\n"
        "  curveBrake: 1 = curve brake activo (s_civicoCurveBrake=true) — speed capado a\n"
        "             SPEED_CIVICO_TURN=25. Activado quando deltaH>0.35rad, desactivado a <0.20rad.\n"
        "  deltaH:    abs(heading road-link - heading veiculo). >0.35=curva, >1.5=WRONG_DIR\n"
        "  mission:   valor de m_nCarMission (eCarMission):\n"
        "               8=GOTOCOORDS(CIVICO/DIRETO) | 11=STOP_FOREVER(PARADO/STOP_ZONE)\n"
        "               12=GOTOCOORDS_ACCURATE\n"
        "               31=ESCORT_REAR | 52=FOLLOWCAR_FARAWAY | 53=FOLLOWCAR_CLOSE\n"
        "               67=ESCORT_REAR_FARAWAY (TODOS descontinuados v5.6 — usa GOTOCOORDS)\n"
        "  CIVICO_F/G/H: mission=8(GOTOCOORDS) destino 15m atras jogador, AVOID_CARS\n"
        ...
        "  targetCar: sempre NULL em v5.6+ (GOTOCOORDS puro, sem escort missions)\n"
        ...
        "\n"
        "  CONDUCAO CIVICO:\n"
        "  Todos os modos CIVICO (F/G/H) usam MISSION_GOTOCOORDS(8) com destino\n"
        "    CIVICO_FOLLOW_OFFSET(15m) atras do jogador + AVOID_CARS.\n"
        "    v5.6: MC52/53/67 descontinuados (causavam posicao lateral e crash).\n"
        "    v5.7: Curve brake restaurado — s_civicoCurveBrake via GetRoadLinkHeading.\n"
        "      Sem brake, GOTOCOORDS passa curvas a 70+ km/h. Cap: SPEED_CIVICO_TURN=25.\n"
        "      VERIFICAR: curveBrake=1 activa em curvas reais, deltaH>0.35 no log?\n"
        "                 speed_ap caido para 25 em curveBrake=1?\n"
        "\n"
        ...
        "  MISSION_RECOVERY: STOP_FOREVER detectado fora das zonas — restaurar GOTOCOORDS.\n"
        "    NOTA: missoes road-graph (31/52/53/67) sao substituidas por GOTOCOORDS em v5.6.\n"
        ...
        "    Nesta zona: speed reduzida (player-8 a 10-15m, player-12 abaixo de 10m).\n"
        "    v5.6: sem force MC52 — GOTOCOORDS mantem controlo directo do destino.\n"
        "  MULTI_TARGET_NULL: m_pTargetCar forcado a nullptr (GOTOCOORDS nao usa targetCar).\n"
        ...
        "========================================================\n"
        "\n"
        "  CIVICO_F (GOTOCOORDS puro) — v5.6\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO:\n"
        "    - GOTOCOORDS(8) SEMPRE: destino 15m atras do jogador (CIVICO_FOLLOW_OFFSET).\n"
        "      v5.6 removeu MC67 (ESCORT_REAR_FARAWAY) por causar posicao LATERAL e crash.\n"
        "    - 5 zonas de velocidade: >40m=70, 30-40m=player+8, 15-30m=player+3,\n"
        "      10-15m=player-8, <10m=player-12.\n"
        "    - Off-road: GOTOCOORDS directo ao jogador (via s_playerOffroadDirect).\n"
        "    - curveBrake REMOVIDO de CIVICO (v5.3).\n"
        "  O QUE VERIFICAR:\n"
        "    + mission=8(GOTOCOORDS) consistente (NUNCA mission=67 ou 31)?\n"
        "    + Recruta fica ATRAS sem ultrapassar ou ficar ao lado?\n"
        "    + DIST_TREND mostra APROXIMAR ou ESTAVEL na maior parte do tempo?\n"
        "    + driveStyle=2(AVOID_CARS) preservado?\n"
        "    + Offroad: recruta segue jogador sem voltar a estrada prematuramente?\n"
        "\n"
        "  CIVICO_G (GOTOCOORDS puro) — v5.6\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.3+ unificou todos os modos).\n"
        "    GOTOCOORDS(8) com destino 15m atras do jogador.\n"
        "  O QUE VERIFICAR:\n"
        ...
        "    + STUCK_RECOVER frequente = colidindo com bordas de passeio/props.\n"
        "    + DIST_TREND: recruta mantem distancia razoavel ou fica sempre muito proximo?\n"
        "\n"
        "  CIVICO_H (GOTOCOORDS puro) — v5.6 [MELHOR MODO]\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.3+ unificou todos os modos).\n"
        "    GOTOCOORDS(8) com destino 15m atras do jogador.\n"
        "    CLOSE_BLOCKED_WAIT continua activo em CIVICO_H.\n"
        "  O QUE VERIFICAR:\n"
        ...
        "\n"
        "  v5.1 MUDANCAS:\n"
        "  10. CURVE BRAKE ROAD-LINK: detecta curvas REAIS da estrada via\n"
        "      ClipTargetOrientationToLink (heading do road segment), em vez da\n"
        "      direcao ao waypoint. Resolve curve brake permanente em retas que\n"
        "      nao alinham com o waypoint distante. Fallback off-road: heading jogador.\n"
        "      VERIFICAR: curveBrake=1 activa APENAS em curvas reais, nao em retas?\n"
        "      deltaH nos logs AI deve ser ~0 em retas e >0.35 em curvas.\n"
        "\n"
        "  11. GOTOCOORDS PURO (v5.6): ESCORT_REAR_FARAWAY (MC67) descontinuado.\n"
        "      Todos os modos CIVICO usam MISSION_GOTOCOORDS(8) com destino 15m atras.\n"
        "      Fix crash 0x004279E4 (null+offset via m_pTargetCar de missoess escort).\n"
        "      Fix posicao lateral (MC67 usava road-graph que navegava pela faixa contraria).\n"
        "      VERIFICAR: mission=8(GOTOCOORDS) sempre; recruta fica ATRAS do jogador.\n"
        "\n"
        "  12. REPARACAO VISUAL SEGURA (sem ESP crash):\n"
        "      Apenas escrita directa a campos (m_nPanelsStatus, m_nLightsStatus,\n"
        "      m_nDoorStatus, SetDoorStatus). Sem CloseAllDoors/FixDoor/FixPanel.\n"
        "      v5.3: Intervalo reduzido 2s→1s para fechar portas mais rapido.\n"
        "      bCanBeDamaged=false re-aplicado a cada reparacao.\n"
        "\n"
        "  v5.3 MUDANCAS:\n"
        "  14. MC67 EM VEZ DE MC31 (v5.3, ambos descontinuados v5.6):\n"
        "      Log v5.1 mostrou MC31 (ESCORT_REAR) a forcar driveStyle=STOP_FOR_CARS.\n"
        "      MC67 preservava AVOID_CARS; v5.6 removeu ambos — GOTOCOORDS resolve tudo.\n"
        "      VERIFICAR: driveStyle=2(AVOID_CARS) em todos os modos CIVICO.\n"
        "\n"
        "  15. OFFROAD AVOID_CARS: s_playerOffroadDirect agora usa AVOID_CARS em vez\n"
        "      de STOP_IGNORE_LIGHTS. Log v5.1 mostrou recruta a 62m com physSpeed=0\n"
        "      — completamente parado por STOP_IGNORE_LIGHTS atras de obstaculo.\n"
        "      VERIFICAR: recruta desvia de obstaculos quando jogador esta offroad.\n"
        "\n"
        "  16. ENTRY CLEAR REVERSE: tempAction limpo apos SetupDriveMode\n"
        "      quando recruta entra como condutor. Previne reverso automatico.\n"
        "      NOTA: m_nTimeTempAction NAO existe no plugin-sdk CAutoPilot.\n"
        "      VERIFICAR: recruta parte para a frente ao entrar no carro.\n"
        "\n"
        "  17. CLOSE-RANGE SPEED: 5 zonas refinadas (v5.4/v5.5):\n"
        "      >40m: SPEED_PASSENGER(70) catch-up | 30-40m: playerSpeed+8 cap 70\n"
        "      15-30m: playerSpeed+3 cap 46 | 10-15m: playerSpeed-8 cap 46\n"
        "      <10m: playerSpeed-12 cap 46 (desaceleracao forte)\n"
        "      v5.5: 10-15m reduzido -5 -> -8; <10m reduzido -8 -> -12.\n"
        "      VERIFICAR: recruta desacelera progressivamente ao aproximar.\n"
        "\n"
        "  18. VELOCIDADES AUMENTADAS: SPEED_CIVICO_HIGH 60->70, CATCHUP 55->62, FAR 70->80.\n"
        "      v5.6: GOTOCOORDS nao depende do road-graph para curvas — velocidades mantidas.\n"
        "      VERIFICAR: recruta acompanha melhor em retas sem problemas em curvas.\n"
        "\n"
        "  19. DOOR REPAIR 1s: CAR_VISUAL_FIX_INTERVAL reduzido 2s→1s.\n"
        "      SetDoorStatus loop (m_nDoorStatus NAO existe em CDamageManager do plugin-sdk).\n"
        "      RepairCarVisualDamage chamada imediatamente ao entrar no carro.\n"
        "      VERIFICAR: portas fecham mais rapido apos colisoes.\n"
        "\n"
        "  v5.4 MUDANCAS:\n"
        "  20. FIX BUILD: m_nDoorStatus removido (nao existe em CDamageManager do plugin-sdk).\n"
        "      m_nTimeTempAction removido (nao existe em CAutoPilot do plugin-sdk).\n"
        "\n"
        "  21. CLOSE-RANGE SPEED: <15m agora usa playerSpeed-5 (10-15m) e playerSpeed-8\n"
        "      (<10m) — recruta anda mais LENTO que jogador para criar gap natural.\n"
        "      Log v5.3 mostrou recruta a bater atras a 10-13m com speed=playerSpeed.\n"
        "      VERIFICAR: recruta mantem 10-15m sem bater atras.\n"
        "\n"
        "  22. REVERSE PER-FRAME CLEAR: tempAction=REVERSE/REVERSE_LEFT/REVERSE_RIGHT limpo\n"
        "      per-frame em PASSENGER e CIVICO GOTOCOORDS. SA engine SetTempAction(REVERSE)\n"
        "      armazena duracao mas NUNCA a verifica para timeout (gta-reversed confirmou).\n"
        "      REVERSE persiste indefinidamente sem ClearTempAct() explicito.\n"
        "      v5.12: estendido para CIVICO — log v5.11 mostrou REVERSE 4+s (dist 74→95m).\n"
        "      VERIFICAR: recruta ja nao anda de marcha-atras em PASSENGER ou CIVICO.\n"
        "\n"
        "  23. CAR PERSISTENCE: bStreamingDontDelete=true no carro do recruta (primario e\n"
        "      secundarios). Previne despawn pelo streaming engine quando jogador se afasta.\n"
        "      Restaurado a false em DismissRecruit e quando recruta secundario sai do carro.\n"
        "      v5.5 FIX: bScriptDontDelete nao existe em CVehicle do plugin-sdk.\n"
        "      Campo correcto: bStreamingDontDelete (herdado de CEntity).\n"
        "      VERIFICAR: carro do recruta fica estacionado quando recruta sai.\n"
        "\n"
        "  24. CIVICO_FOLLOW_OFFSET 10->15m: ponto-alvo mais longe atras do jogador.\n"
        "      v5.6: offset aplicado no GOTOCOORDS — recruta navega DIRECTAMENTE 15m atras.\n"
        "      VERIFICAR: recruta para ~15m atras do jogador (nao ao lado).\n"
        "\n"
        "  v5.5 MUDANCAS:\n"
        "  25. FIX BUILD: bScriptDontDelete→bStreamingDontDelete em todos os ficheiros.\n"
        "      bScriptDontDelete nao e membro de CVehicle no plugin-sdk (dk22pac).\n"
        "      bStreamingDontDelete e herdado de CEntity — mesmo efeito: previne despawn.\n"
        "\n"
        "  26. OFFROAD PLAYER CHECK (v5.5, melhorado v5.6).\n"
        "\n"
        "  27. GOTOCOORDS PURO (v5.6): MC67 per-frame incondicional (v5.5) removido.\n"
        "      SetupDriveModeSimple actualizado: recrutas secundarios tambem usam GOTOCOORDS.\n"
        "      MULTI_TARGET_CAR_UPDATE removido: m_pTargetCar=nullptr em todos os recrutas.\n"
        "      OFFROAD_EXIT_SNAP: skip quando jogador ainda esta offroad (s_playerOffroadDirect).\n"
        "\n"
        "  28. CLOSE-RANGE SPEED: Reducoes mais agressivas:\n"
        "      10-15m: playerSpeed-8 (era -5) | <10m: playerSpeed-12 (era -8)\n"
        "      Log v5.4 mostrou recruta ainda a bater atras a 10-13m com gap insuficiente.\n"
        "      VERIFICAR: recruta desacelera visivelmente ao aproximar, sem colisao traseira.\n"
        "\n"
        "  v5.6 MUDANCAS:\n"
        "  29. MC67 REMOVIDO — GOTOCOORDS PURO: MC_ESCORT_REAR_FARAWAY (67) descontinuado.\n"
        "      Log v5.5 demonstrou problemas fundamentais com MC67:\n"
        "      a) POSICAO LATERAL: MC67 usa road-graph que navega pela faixa contraria\n"
        "         em estradas de 2 faixas. deltaH=-2.310(WRONG_DIR!) frequente no log.\n"
        "         Recruta ficava ao LADO ou FRENTE em vez de ATRAS do jogador.\n"
        "      b) OSCILACAO RAPIDA: onRoad flipa a cada 2 frames perto de fronteiras\n"
        "         estrada/offroad → ESCORT↔GOTOCOORDS oscila → estado autopilot corrupto.\n"
        "         Crash a 0x004279E4 lendo 0x000000DD (null pointer + offset).\n"
        "      c) GOTOCOORDS com destino CIVICO_FOLLOW_OFFSET (15m) atras do jogador\n"
        "         resolve ambos os problemas: posicao directa, sem dependencia de grafo.\n"
        "      VERIFICAR: mission=8(GOTOCOORDS) consistente, recruta ATRAS do jogador.\n"
        "\n"
        "  30. FIX OFFROAD: Recruta voltava a estrada sem jogador ter voltado.\n"
        "      v5.5 usava s_lastPlayerRoadDist < OFFROAD_ON_DIST_M(20m) — threshold\n"
        "      muito alto, quase sempre verdadeiro. Jogadores perto de estradas estarao\n"
        "      quase sempre dentro de 20m de um nó.\n"
        "      v5.6 usa !s_playerOffroadDirect (flag com hysteresis activada a >22m,\n"
        "      desactivada a <18m com sustain de 30 frames). Mais preciso e estavel.\n"
        "      VERIFICAR: recruta segue jogador offroad sem voltar a estrada.\n"
        "\n"
        "  v5.6.1 MUDANCAS:\n"
        "  31. FIX BUILD: GetPlayerForwardVec (nao existia) substituido por\n"
        "      inline playerCar->GetHeading() + sinf/cosf em SetupDriveMode.\n"
        "      Sem este fix, MSVC dava erro C3861 e continuava a usar .asi antigo.\n"
        "\n"
        "  v5.7 MUDANCAS:\n"
        "  32. CURVE BRAKE RESTAURADO PARA CIVICO GOTOCOORDS:\n"
        "      MC67 usava road-graph que trata curvas nativamente. GOTOCOORDS puro nao.\n"
        "      Sem brake: recruta passa curvas a 70+ km/h, sobe calcada, ultrapassa jogador.\n"
        "      Fix: s_civicoCurveBrake via GetRoadLinkHeading + hysteresis identica a\n"
        "      PASSENGER/WAYPOINT (activar >0.35 rad, desactivar <0.20 rad).\n"
        "      Cap: se s_civicoCurveBrake=true, speed = min(speed, SPEED_CIVICO_TURN=25).\n"
        "      Nao substitui zonas de distancia — apenas cap; nunca aumenta velocidade.\n"
        "      Log: CIVICO_DRIVE_1 agora inclui curveBrake=0/1 e deltaH=X.XXX.\n"
        "\n"
        "  v5.8-v5.11 MUDANCAS:\n"
        "  33. LATERAL APPROACH PREVENTION (v5.8-v5.11):\n"
        "      Recruta embicar para o lado ao aproximar-se do jogador.\n"
        "      v5.8: retreat offset (destino atras do recruta) — causava oscilacao.\n"
        "      v5.10: thresholds mais estritos (ALIGN_DOT 0.5→0.7, CLOSE_ALIGN 20→25m).\n"
        "      v5.11: ALIGN_DOT 0.7→0.75, CLOSE_ALIGN 25→30m, SLOW_ZONE 10→12m.\n"
        "\n"
        "  v5.12 MUDANCAS:\n"
        "  34. REVERSE CLEARING PER-FRAME EM CIVICO:\n"
        "      SA engine SetTempAction(REVERSE) NUNCA faz timeout (gta-reversed confirmou).\n"
        "      Log v5.11: REVERSE por 4+s, recruta de 74m a 95m. Fix: limpar per-frame.\n"
        "  35. CURVE BRAKE DESTINATION-VECTOR:\n"
        "      Road-link heading NAO reflecte direccao real em GOTOCOORDS.\n"
        "      Log v5.11: curveBrake=1 em 95%% entries (deltaH 1-3 rad). Fix: usar heading\n"
        "      ao destino com thresholds CIVICO-especificos (0.60/0.35 vs 0.35/0.20).\n"
        "  36. LATERAL FIX SIMPLIFICADO:\n"
        "      v5.8-v5.11 retreat offset causava oscilacao. v5.12: quando desalinhado,\n"
        "      apenas reduzir velocidade a SPEED_SLOW. Jogador puxa a frente naturalmente.\n"
        "  37. PERIODIC ROAD SNAP RESTAURADO (60 frames, 1s).\n"
        "\n"
        "  v5.13 MUDANCAS:\n"
        "  38. LANE HOLD (FIX SIDE-BY-SIDE):\n"
        "      Quando jogador parado (<3kmh) e recruta proximo (<30m):\n"
        "      STOP_FOREVER na faixa actual. GOTOCOORDS+AVOID_CARS com destino 20m atras\n"
        "      causa SA engine a rotear por faixas adjacentes. Com STOP_FOREVER, recruta\n"
        "      fica onde esta. Retoma quando jogador anda (>8kmh).\n"
        "      Activado em TODOS os modos CIVICO (era apenas CIVICO_H).\n"
        "      Log: CIVICO_LANE_HOLD_ON / CIVICO_LANE_HOLD_OFF.\n"
        "      VERIFICAR: recruta para atras do jogador parado? Nao embicar para o lado?\n"
        "  39. DEST UPDATE SUAVE (FIX NAVEGACAO CIVICO):\n"
        "      PASSENGER actualiza destino ~nunca (waypoint fixo) -> navegacao superior.\n"
        "      CIVICO v5.12 actualizava a cada frame (stale>3m) E resetava m_nTempAction\n"
        "      a cada update -> interrompia manobras de desvio -> jitter.\n"
        "      Fix: (a) CIVICO_DEST_STALE_DIST 3m->8m, (b) timer minimo 30 frames (0.5s)\n"
        "      entre updates, (c) NAO resetar m_nTempAction em updates de destino.\n"
        "      REVERSE limpo per-frame separadamente.\n"
        "      VERIFICAR: navegacao mais suave? Menos jitter em curvas? Menos batidas?\n"
        "  40. TELEPORT CATCH-UP (FIX DESPAWN):\n"
        "      Quando recruta >150m do jogador: warpar carro para 30m atras (off-screen).\n"
        "      Usa CEntity::Teleport() (gta-reversed: Remove do world, reset physics,\n"
        "      re-Add). Apos: JoinCarWithRoadSystem para alinhar com road-graph.\n"
        "      Cooldown 5s entre teleports. Resolve despawn sem velocidades altas.\n"
        "      Log: CIVICO_TELEPORT_CATCHUP.\n"
        ...
        "\n"
        "  v5.14 MUDANCAS:\n"
        "  42. FIX VECTOR LATERAL APPROACH (BUG CRITICO v5.8-v5.13):\n"
        "      Vector de alinhamento estava INVERTIDO: usava vPos-playerPos (player→recruta)\n"
        "      em vez de playerPos-vPos (recruta→jogador). Consequencia:\n"
        "        - Quando recruta ATRAS do jogador (posicao CORRECTA): dot≈-1.0 → activava\n"
        "          slowdown para 12kmh ERRONEAMENTE → recruta ficava a 28-30m sempre.\n"
        "        - Log v5.13 mostrou alignDot=-1.00 activando slowdown com recruta atras.\n"
        "      FIX: vector corrigido para playerPos-vPos (recruta→jogador):\n"
        "        - Recruta atras: dot≈+1.0 → SEM slowdown (correcto!)\n"
        "        - Recruta ao lado: dot≈0.0 → slowdown activo (correcto!)\n"
        "        - Recruta a frente: dot≈-1.0 → slowdown activo (correcto!)\n"
        "      VERIFICAR: recruta aproxima-se naturalmente ate ~20m (CIVICO_FOLLOW_OFFSET).\n"
        "      VERIFICAR: CIVICO_LATERAL_SLOWDOWN so aparece quando recruta ao LADO.\n"
        "      VERIFICAR: log mostra pos=quase_atras/ao_lado/a_frente correctamente.\n"
        "  43. FIX MODO AGRESSIVO (BUG v4.9-v5.13):\n"
        "      Em modo agressivo, RESCAN (cada 2s) suprimia TellGroupFollow (emitFollow=false)\n"
        "      para 'nao interromper combate'. Consequencia:\n"
        "        - Apos burst inicial (5s), recruta NUNCA recebia follow → STAND_STILL(203).\n"
        "        - Log v5.13 mostrou aggr=1 doFollow=0 com [4]STAND_STILL permanente.\n"
        "        - Recruta ficava parado enquanto jogador se afastava (dist 37→76m).\n"
        "      FIX: RESCAN agora emite follow em TODOS os modos (emitFollow=true sempre).\n"
        "      TellGroupToStartFollowingPlayer(aggressive=true) permite combate E follow.\n"
        "      Evento GATHER so dispara SeekEntity se recruta sem tarefa activa → seguro.\n"
        "      VERIFICAR: recruta segue jogador em modo agressivo E ataca inimigos.\n"
        "      VERIFICAR: recruta nao fica em STAND_STILL(203) apos burst inicial.\n"
        "  44. FIX TOGGLE AGRESSIVO/PASSIVO (EFEITO IMEDIATO):\n"
        "      KEY N e MENU aggro so chamavam ForceGroupToAlwaysFollow, sem TellGroupFollow.\n"
        "      Recruta so mudava comportamento no proximo RESCAN (~2s de atraso).\n"
        "      FIX: toggle agora tambem emite TellGroupFollowWithRespect imediatamente.\n"
        "      VERIFICAR: ao premir N, recruta muda de passivo↔agressivo instantaneamente.\n"
        "  45. LOGGING MELHORADO PARA ITERACAO COM AGENTE:\n"
        "      - CIVICO_LATERAL_SLOWDOWN agora mostra pos=quase_atras/ao_lado/a_frente.\n"
        "      - RESCAN mostra decisao de follow com razao.\n"
        "      - Toggle aggr mostra TellGroupFollow emitido.\n"
        "      - Changelog v5.14 no cabecalho do log para contexto rapido.\n"
        "  46. v5.18 FOLLOWCAR (CORRIGE TUDO: FAIXA, CRUZAMENTOS, PARAGEM):\n"
        "      PROBLEMA v5.6-v5.17: GOTOCOORDS com destino 20m atras do jogador fazia o\n"
        "        pathfinder (CCarAI) rotear o recruta por cruzamentos errados para chegar\n"
        "        a coordenada. v5.17 AHEAD_HOLD piorou: recruta parado 60m atras do\n"
        "        jogador porque alignDot ficava negativo em estradas curvas (falso positivo).\n"
        "      FIX: FOLLOWCAR_FARAWAY(52) — missao nativa do SA engine para seguir carro.\n"
        "        - Segue o CAMINHO do carro do jogador pelo road-graph (nao coordenada fixa)\n"
        ...
        "      REMOVEU: GOTOCOORDS destino-atras, AHEAD_HOLD, CIVICO curve brake,\n"
        "        destUpdateTimer, alignDot, CIVICO_FOLLOW_OFFSET para seguimento.\n"
        "      Referencia gta-reversed: CCarAI::UpdateCarAI processa FOLLOWCAR.\n"
        "  47. v5.19 FOLLOWCAR SPEED + SLOW_ZONE FIX:\n"
        "      BUG 1 — SLOW_ZONE GOTOCOORDS: SLOW_ZONE (6-12m) sobrescrevia FOLLOWCAR(52)\n"
        "        com GOTOCOORDS(8)+targetCar=null. Causava oscilacao missao a cada frame\n"
        "        que recruta entrava/saia da SLOW_ZONE. Log v5.18 mostrou\n"
        "        CIVICO_FOLLOWCAR_RESTORE: mission 9(?)->52 repetido.\n"
        "        FIX: SLOW_ZONE agora apenas reduz cruiseSpeed=25. Para restaurar FOLLOWCAR\n"
        "        apos STOP_ZONE, usa MC_FOLLOWCAR_FARAWAY (nao GOTOCOORDS).\n"
        "      BUG 2 — speed_ap=8 com jogador parado: playerSpeed-based speed zones davam\n"
        "        target = 0 + 8 = 8 quando jogador parado a 30m.\n"
        "        FIX: Zonas fixas por distancia: <40m=46, 40-60m=62, 60-80m=80, >80m=85.\n"
        "  48. v5.20 CURVE BRAKE + CLOSE DISTANCE + DRIVESTYLE FIX:\n"
        "      BUG 1 — CURVAS RAPIDAS: v5.18 removeu curve brake para CIVICO acreditando\n"
        "        que FOLLOWCAR limitava velocidade em curvas nativamente. Log v5.19 mostrou\n"
        "        physSpeed=100kmh em curvas (heading=1.2 rad). Recruta cortava calcadas.\n"
        "        FIX: Re-activado curve brake CIVICO usando road-link heading (mesma tecnica\n"
        "        que PASSENGER mode que funciona perfeitamente). Thresholds CIVICO (0.80/0.50)\n"
        "        mais altos que PASSENGER (0.35/0.20) + limite superior 1.80 rad.\n"
        "      BUG 2 — PARAR AO LADO: recruta chegava a alta velocidade de curva e parava\n"
        "        ao lado do jogador. speed_ap=46 para dist<40m era alto demais para <20m.\n"
        "        FIX: Nova zona <20m com speed=SPEED_SLOW(25). FOLLOWCAR desacelera atras.\n"
        "      BUG 3 — CORTAR CALCADA: AVOID_CARS(2) faz swerve por calcada em curvas.\n"
        "        FIX: DriveStyle adaptativo — STOP_FOR_CARS(0) quando dist<20m (trafego\n"
        "        normal para atras), AVOID_CARS(2) quando dist>20m (catch-up sem parar).\n"
        "        Referencia gta-reversed: CCarAI — trafego normal usa driveStyle 0.\n"
        "  49. v5.21 STEER-ANGLE BRAKE + SLOW_ZONE FIX + OFFROAD STOPPED + RTF COMPAT:\n"
        "      ESTUDO: real-traffic-fix (JuniorDjjr) estudado a fundo. Tecnicas chave:\n"
        "        - fNewSpeed -= cfgTurningSpeedDecrease(15) * absSteerAngle (reducao proporcional)\n"
        "        - ProcessLineOfSight 3 raios frente (C/L/R) para detectar obstaculos\n"
        "        - freeLineTimer para aceleracao suave em linha recta\n"
        "        - FILTRO: m_nCreatedBy != MISSION_VEHICLE — NOSSAS VIATURAS SA FILTRADAS!\n"
        "      BUG CRITICO — RTF OVERRIDE DE VELOCIDADE:\n"
        "        RTF (vehicleRenderEvent.before) faz: m_nCruiseSpeed = cfgCruiseSpeed(15)\n"
        "        ANTES do check de m_pTargetCar. Mesmo em FOLLOWCAR mode, o cruiseSpeed\n"
        "        e sobreescrito para 15 por um frame inteiro — SA engine ve 15 em vez de\n"
        "        46-85kmh. Resultado: recruta arrastava-se a ~8kmh reais intermitentes.\n"
        "        FIX: g_car->m_nCreatedBy = 2 (MISSION_VEHICLE) no setup E per-frame.\n"
        "        RTF skipa completamente carros com m_nCreatedBy == MISSION_VEHICLE.\n"
        "        VERIFICAR: log CAR_DURABILITY_SETUP contem 'createdBy=2 RTF_BLOCKED=1'.\n"
        "      BUG 1 — SLOW_ZONE sem driveStyle: SLOW_ZONE (dist<12m) retornava sem\n"
        "        definir driveStyle. driveStyle ficava em AVOID_CARS do frame anterior\n"
        "        (quando dist era >30m). Recruta desviava pela calcada mesmo a <12m.\n"
        "        FIX: SLOW_ZONE define STOP_FOR_CARS em modo CIVICO (RTF-inspired).\n"
        "        STOP_ZONE tambem define STOP_FOR_CARS para consistencia na transicao.\n"
        "      BUG 2 — STOP_FOR_CARS threshold pequeno (20m): CIVICO block definia\n"
        "        AVOID_CARS a >20m. Com curvas entre 20-30m o recruta cortava calcadas.\n"
        "        FIX: Novo CIVICO_STOP_FOR_CARS_DIST=30m (era CIVICO_CLOSE_ALIGN_DIST=20m).\n"
        "        AVOID_CARS so activo a >30m (catch-up puro). RTF usa STOP_FOR_CARS sempre.\n"
        "      BUG 3 — CURVAS RAPIDAS (deltaH threshold alto): threshold era 0.80 rad.\n"
        "        Log v5.19 mostrou deltaH=0.784-0.853 — muitos valores NAO trigavam.\n"
        "        Recruta ia a 75-93kmh em curvas. FIX: ACT 0.80→0.55, DEACT 0.50→0.30.\n"
        "      FIX 4 — STEER-ANGLE SPEED REDUCTION (RTF-inspired):\n"
        "        speedCap = max(SPEED_CIVICO_TURN, speed * (1.0 - absSteerAngle)).\n"
        "        Threshold CIVICO_STEER_BRAKE_MIN=0.15 filtra micro-correccoes.\n"
        "        Aplicado em FOLLOWCAR, OFFROAD_DIRECT, PLAYER_OFFROAD_DIRECT.\n"
        "        Ex: speed=80, steer=0.4 → cap=48. speed=80, steer=0.7 → cap=25.\n"
        ...
        "        VERIFICAR: recruta NAO corta calcada em curvas a 15-30m do jogador.\n"
        "\n"
        "  13. REMOVIDO m_vecMoveSpeed DAMPING:\n"
        "      v5.0 aplicava veh->m_vecMoveSpeed *= 0.97 durante curveBrake.\n"
        "      Isto capava physSpeed a ~40kmh permanentemente (curveBrake ficava ON).\n"
        "      v5.1 usa apenas cruiseSpeed do SA engine para controlo de velocidade.\n"
        "\n"
        "========================================================\n\n",
