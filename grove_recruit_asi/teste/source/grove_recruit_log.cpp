...
void LogInit()
{
    ...
    fprintf(g_logFile,
        ...
        "========================================================\n"
        "\n"
        "  CIVICO_F (ESCORT_REAR+GOTOCOORDS hibrido) — v5.1\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO:\n"
        "    - Perto + on-road (dist<25m, linkId OK): ESCORT_REAR(31).\n"
        "      SA engine posiciona recruta ATRAS do jogador via road-graph.\n"
        "      Resolve: ultrapassagem, colisao traseira, posicionamento lateral.\n"
        "      Usa STOP_FOR_CARS para conducao conservadora.\n"
        "    - Longe (dist>25m): GOTOCOORDS(8) ao ponto 10m atras do jogador.\n"
        "      Catch-up com AVOID_CARS, velocidade ate 70kmh.\n"
        "    - Off-road (linkId INVALID): GOTOCOORDS ao ponto atras do jogador.\n"
        "    - Curve brake: detecta curvas via heading do road-link (nao waypoint).\n"
        "      curveBrake=1 → cruiseSpeed=20kmh. Hysteresis act=0.35 deact=0.20.\n"
        "  O QUE VERIFICAR:\n"
        "    + mission=31(ESCORT_REAR) quando perto, mission=8(GOTOCOORDS) quando longe?\n"
        "    + Recruta fica ATRAS sem ultrapassar ou ficar ao lado?\n"
        "    + Curve brake activa em curvas reais (nao em retas)?\n"
        ...
        "\n"
        "  CIVICO_G (ESCORT_REAR+GOTOCOORDS hibrido) — v5.1\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.1 unificou todos os modos).\n"
        "    ESCORT_REAR(31) perto, GOTOCOORDS(8) longe/off-road.\n"
        "  O QUE VERIFICAR:\n"
        ...
        "\n"
        "  CIVICO_H (ESCORT_REAR+GOTOCOORDS hibrido) — v5.1 [MELHOR MODO]\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.1 unificou todos os modos).\n"
        "    ESCORT_REAR(31) perto, GOTOCOORDS(8) longe/off-road.\n"
        ...
        "\n"
        "  11. CIVICO HIBRIDO ESCORT_REAR + GOTOCOORDS:\n"
        "      - Close + on-road (dist<25m, linkId valido): ESCORT_REAR(31).\n"
        "        O SA engine posiciona recruta ATRAS do jogador naturalmente.\n"
        "        Resolve ultrapassagem, colisao, ficar ao lado.\n"
        "      - Far (dist>25m) ou off-road: GOTOCOORDS(8) para catch-up.\n"
        "      VERIFICAR: mission=31(ESCORT_REAR) aparece no log quando perto?\n"
        "        Recruta fica ATRAS sem ultrapassar? Segue curvas do jogador?\n"
        "        mission=8(GOTOCOORDS) quando longe? Catch-up funciona?\n"
        "      RecruitVsNPC log tem: escort=1/0 onRoad=1/0 para diagnostico.\n"
        "\n"
        "  12. REPARACAO VISUAL SEGURA (sem ESP crash):\n"
        "      Apenas escrita directa a campos (m_nPanelsStatus, m_nLightsStatus,\n"
        "      SetDoorStatus). Sem CloseAllDoors/FixDoor/FixPanel (causavam crash).\n"
