...
void LogInit()
{
    ...
    fprintf(g_logFile,
        ...
        "========================================================\n"
        "\n"
        "  CIVICO_F (ESCORT_REAR_FARAWAY+GOTOCOORDS hibrido) — v5.3\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO:\n"
        "    - Perto + on-road (dist<50m, linkId OK, !offroad): MC67(ESCORT_REAR_FARAWAY).\n"
        "      SA engine segue road-graph ate posicao atras do jogador.\n"
        "      m_nStraightLineDistance=5 previne transicao prematura MC67→MC31.\n"
        "      AVOID_CARS preservado (MC31 forcava STOP_FOR_CARS via engine override).\n"
        "      Resolve: ultrapassagem, colisao traseira, posicionamento lateral.\n"
        "    - Longe (dist>50m): GOTOCOORDS(8) ao ponto 10m atras do jogador.\n"
        "      Catch-up com AVOID_CARS, velocidade ate 70kmh.\n"
        "    - Off-road (g_isOffroad ou linkId INVALID): GOTOCOORDS ao ponto atras.\n"
        "    - curveBrake REMOVIDO de CIVICO (SA engine trata curvas em MC67).\n"
        "  O QUE VERIFICAR:\n"
        "    + mission=67(ESCORT_REAR_FARAWAY) quando perto, mission=8(GOTOCOORDS) quando longe?\n"
        "    + Recruta fica ATRAS sem ultrapassar ou ficar ao lado?\n"
        "    + straightLineDist=5 nos logs AI?\n"
        ...
        "    + driveStyle=2(AVOID_CARS) preservado (nao 0=STOP_FOR_CARS)?\n"
        "\n"
        "  CIVICO_G (ESCORT_REAR_FARAWAY+GOTOCOORDS hibrido) — v5.3\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.3 unificou todos os modos).\n"
        "    MC67(ESCORT_REAR_FARAWAY) perto, GOTOCOORDS(8) longe/off-road.\n"
        "  O QUE VERIFICAR:\n"
        ...
        "\n"
        "  CIVICO_H (ESCORT_REAR_FARAWAY+GOTOCOORDS hibrido) — v5.3 [MELHOR MODO]\n"
        "  --------------------------------------------------------\n"
        "  COMPORTAMENTO ESPERADO: Mesmo que CIVICO_F (v5.3 unificou todos os modos).\n"
        "    MC67(ESCORT_REAR_FARAWAY) perto, GOTOCOORDS(8) longe/off-road.\n"
        ...
        "\n"
        "  26. OFFROAD PLAYER CHECK: useEscort agora verifica se o JOGADOR esta proximo\n"
        "      de estrada (s_lastPlayerRoadDist < OFFROAD_ON_DIST_M=20m). Log v5.4 mostrou\n"
        "      recruta a oscilar GOTOCOORDS↔MC67 quando offroad perto de cruzamentos:\n"
        "      recruta passava perto de nó → g_isOffroad=false → MC67 → volta a estrada.\n"
        "      Com esta verificacao, recruta mantem GOTOCOORDS enquanto jogador offroad.\n"
        "      VERIFICAR: recruta segue jogador sem voltar a estrada prematuramente.\n"
        "\n"
        "  27. MC67 PER-FRAME INCONDICIONAL: MC_ESCORT_REAR_FARAWAY forcado per-frame\n"
        "      sem check condicional. Log v5.4 mostrou SA engine a transicionar MC67→MC31\n"
        "      mid-frame — MC31 ficava activo 1 frame com STOP_FOR_CARS antes de re-set.\n"
        "      VERIFICAR: mission=67 consistente no log, straight=3 (nao 20).\n"
        "\n"
