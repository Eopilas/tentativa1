...
void LogInit()
{
    ...
    fprintf(g_logFile,
        ...
        "    - Jogador prime key 5: recruta muda para WAYPOINT_SOLO e conduz ao waypoint do mapa.\n"
        "    - Jogador pode seguir o recruta no proprio carro (recruta NAO segue o jogador).\n"
        "    - Sem waypoint: recruta vai 40m a frente na direcao do jogador (PLAYER_AHEAD).\n"
        "    - Ao chegar (dist < 10m): recruta actualiza destino imediatamente.\n"
        "    - Log: WAYPOINT_SOLO_NAV (source=MAP_WAYPOINT ou PLAYER_AHEAD).\n"
        "    - Log periodico: WAYPOINT_SOLO_DRIVING (speed_ap, physSpeed, curveBrake, deltaH, distToWaypoint).\n"
        "    - Key 5 novamente: volta ao modo DRIVING (recruta segue o jogador).\n"
        "  O QUE VERIFICAR:\n"
        "    + Recruta navega correctamente ao waypoint? Log WAYPOINT_SOLO_NAV com source=MAP_WAYPOINT?\n"
        "    + curveBrake=1 aparece nas curvas? Recruta abranda suficientemente?\n"
